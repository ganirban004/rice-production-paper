...
trend_df <- rice_df %>% 
  group_by(Year) %>% summarise(Aus = mean(Aus), Aman = mean(Aman), Boro = mean(Boro), 
                               min_T.Aus = mean(min_T.Aus), min_T.Aman = mean(min_T.Aman), 
                               min_T.Boro = mean(min_T.Boro), max_T.Aus = mean(max_T.Aus), 
                               max_T.Aman = mean(max_T.Aman), max_T.Boro = mean(max_T.Boro), 
                               rainfall.Aus = mean(rainfall.Aus), 
                               rainfall.Aman = mean(rainfall.Aman), 
                               rainfall.Boro = mean(rainfall.Boro))
# Productivity #
line_prod <- ggplot(trend_df, aes(x = Year)) + 
  ...
  labs(title = "Productivity", x = NULL, y = "Average Productivity (tonnes/hec)") + 
  theme_minimal() + theme(panel.border = element_rect(color = "black", fill = NA, size = 1), 
                          legend.position = "none", axis.text.x = element_blank())
# Minimum Temperature #
line_minT <- ggplot(trend_df, aes(x = Year)) +
  geom_line(aes(y = min_T.Aus, color = "min_T.Aus"), size = 0.7) + 
  geom_point(aes(y = min_T.Aus, color = "min_T.Aus")) +
  geom_line(aes(y = min_T.Aman, color = "min_T.Aman"), size = 0.7) + 
  geom_point(aes(y = min_T.Aman, color = "min_T.Aman")) +
  geom_line(aes(y = min_T.Boro, color = "min_T.Boro"), size = 0.7) + 
...
line_maxT <- ggplot(trend_df, aes(x = Year)) +
  geom_line(aes(y = max_T.Aus, color = "max_T.Aus"), size = 0.7) + 
  geom_point(aes(y = max_T.Aus, color = "max_T.Aus")) +
  geom_line(aes(y = max_T.Aman, color = "max_T.Aman"), size = 0.7) + 
  geom_point(aes(y = max_T.Aman, color = "max_T.Aman")) +
  geom_line(aes(y = max_T.Boro, color = "max_T.Boro"), size = 0.7) + 
...
line_rainfall <- ggplot(trend_df, aes(x = Year)) +
  geom_line(aes(y = rainfall.Aus, color = "Aus"), size = 0.7) + 
  geom_point(aes(y = rainfall.Aus, color = "Aus")) +
  geom_line(aes(y = rainfall.Aman, color = "Aman"), size = 0.7) + 
  geom_point(aes(y = rainfall.Aman, color = "Aman")) +
  geom_line(aes(y = rainfall.Boro, color = "Boro"), size = 0.7) + 
...
violin_aus <- ggplot(rice_df, aes(x = District, y = Aus)) + 
  ...
  labs(title = "Aus", x = NULL, y = "Average Productivity (in tonnes/hec)") + 
  theme_minimal() + theme(panel.border = element_rect(color = "black", fill = NA, size = 1), 
...
violin_aman <- ggplot(rice_df, aes(x = District, y = Aman)) + 
  ...
  stat_summary(fun = mean, geom = "point", color = "black") + 
  stat_summary(fun = mean, geom = "line", aes(group = 1), color = "black") + 
  labs(title = "Aman", x = NULL, y = "Average Productivity (in tonnes/hec)") + 
  theme_minimal() + theme(panel.border = element_rect(color = "black", fill = NA, size = 1), 
        axis.text.x = element_blank())
# Boro #
violin_boro <- ggplot(rice_df, aes(x = District, y = Boro)) + 
  ...
  stat_summary(fun = mean, geom = "point", color = "black") + 
  stat_summary(fun = mean, geom = "line", aes(group = 1), color = "black") + 
  labs(title = "Boro", x = "District", y = "Average Productivity (in tonnes/hec)") + 
  theme_minimal() + theme(panel.border = element_rect(color = "black", fill = NA, size = 1), 
...
re_model_aman <- lm(Aman ~ min_T.Aman + max_T.Aman + rainfall.Aman, data = rice_df)
